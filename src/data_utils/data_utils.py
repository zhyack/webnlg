        ...
        for inp in inputs:
            for outp in outputs:
                ...
                if only_input:
                    ...
                    # fout.write(outp+'\n')
                    # fout.write(_2utf(' '.join(word_tokenize(_2uni(outp))))+'\n')
                    for outcnt in range(8):
                        ftmp = open(pfout+'%d'%outcnt,'a')
                        if outcnt < len(outputs):
                            ftmp.write(_2utf(' '.join(word_tokenize(_2uni(outputs[outcnt]))))+'\n')
                        else:
                            ftmp.write('\n')
                        ftmp.close()
                    for k in t_inp.keys():
...
d1,rd1 = getDict(['data_process_pack/train-webnlg-all-delex.triple'], 'data_process_pack/dict_src_all')
d2,rd2 = getDict(['data_process_pack/train-webnlg-all-delex.lex'], 'data_process_pack/dict_dst_all')
