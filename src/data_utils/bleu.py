...
def bleu_count(hypothesis, references, max_n=4):
    ...
    for i in range(len(hypothesis)):
        ...
        for i in range(n_ref):
            ...
            elif diff==closest_diff and l < closest_length:
                closest_length = y_len[i]
            ...
            for n in range(max_n):
                ...
                for st in range(0, y_len[i]-n):
                    ...
                    for k in range(n):
                        j = st+k
                        ngram += " %s"%(y[i][j])
                    if not sent_ngram.has_key(ngram):
                        sent_ngram[ngram]=0
                    sent_ngram[ngram]+=1
                for ngram in sent_ngram.keys():
                    if not ref_ngram.has_key(ngram) or ref_ngram[ngram]<sent_ngram[ngram]:
        ...
        for n in range(max_n):
            ...
            for st in range(0, x_len-n):
                ...
                for k in range(n):
                    j = st+k
                    ngram += " %s"%(x[j])
                if not hyp_ngram.has_key(ngram):
                    hyp_ngram[ngram]=0
                hyp_ngram[ngram]+=1
            for ngram in hyp_ngram.keys():
                if ref_ngram.has_key(ngram):
...
for pf in args.pf_hypothesis+args.pf_references:
    ...
    if n==None:
        n = len(data[-1])
    elif n != len(f.readlines()):
        raise Exception("Not parrallel: %s"%(pf))
    f.close()

hyp_data = data[0]
ref_data = list(map(list, zip(*data[1:])))

bleu = corpus_bleu(hyp_data, ref_data)

print("BLEU = %.2f, %.1f/%.1f/%.1f/%.1f (BP=%.3f, ratio=%.3f, hyp_len=%d, ref_len=%d)"%(bleu[0], bleu[1], bleu[2], bleu[3], bleu[4]))
